...
private fun articleHandler(
  ...
  env: Env,
  origin: String,
): Promise<Response> = fetchFromAssets("$datePath/index.html", env, origin)

private fun articleAssetHandler(
  datePath: String,
  filename: String,
  env: Env,
  origin: String,
): Promise<Response> = fetchFromAssets("$datePath/$filename", env, origin)

private fun pageHandler(
  num: String,
  env: Env,
  origin: String,
): Promise<Response> = fetchFromAssets("page/$num/index.html", env, origin)

private fun privacyPolicyHandler(
  env: Env,
  origin: String,
): Promise<Response> = fetchFromAssets("privacy_policy.html", env, origin)
...
private fun cacheControlFor(filename: String): String =
  when {
    ...
    filename.endsWith(".gif") || filename.endsWith(".png") || filename.endsWith(".jpg") || filename.endsWith(".jpeg") ||
      filename.endsWith(".ico") -> "public, max-age=31536000, immutable"
